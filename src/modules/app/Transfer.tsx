...
export default function Transfer() {
  ...
  const handleTransfer = useCallback(async () => {
    if (!privacyWallet || !defaultToken) return;

    const transferAmount = parseAmount();
    if (transferAmount === 0n) {
      updateState({ error: 'Invalid transfer amount' });
      return;
    }
    ...
    try {
      ...
      const { createMockTransactProof } = await import('@/src/wallet-sdk/mockProofs');
      const { keccak256, encodeAbiParameters } = await import('viem');

      // Select notes to spend (greedy selection)
      const { notes: selectedNotes, total } = privacyWallet.selectNotesForAmount(defaultToken.address, transferAmount);
      ...
      const recipientCommitment = keccak256(
        encodeAbiParameters(
          ...
          [transferAmount, state.recipientAddress as `0x${string}`, BigInt(Date.now())],
        ),
      );
  ...
  }, [privacyWallet, defaultToken, state.recipientAddress, parseAmount, updateState, refreshBalance]);
    ...
    return (
      <div className='p-6 max-w-xl mx-auto'>
        <div className='bg-zinc-900/50 border border-zinc-800 rounded-xl p-8 text-center'>
          ...
          <p className='text-zinc-500 text-sm'>Please connect your wallet to make private transfers.</p>
        </div>
      </div>
    );
  }
      ...
      <div className='p-6 max-w-xl mx-auto'>
        <div className='bg-zinc-900/50 border border-zinc-800 rounded-xl p-8 text-center'>
          ...
          <p className='text-zinc-500 text-sm mb-4'>
            You don't have any shielded assets to transfer. Make a deposit first.
          </p>
        </div>
      </div>
    );
              ...
              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-colors ${
                state.step === step || state.step === 'processing'
                  ...
                  : ['confirm', 'processing', 'success'].indexOf(state.step) >
                    ['confirm', 'processing', 'success'].indexOf(step as TransferStep)
            ...
            {index < 2 && (
              <div
                className={`flex-1 h-0.5 ${
                  ['confirm', 'processing', 'success'].indexOf(state.step) > index ? 'bg-green-500' : 'bg-zinc-800'
                }`}
              />
            )}
          </div>
      ...
      <motion.div
        ...
        {state.step === 'input' && (
          <>
            {/* Recipient Address */}
            <div className='mb-6'>
              <label className='block text-zinc-500 text-sm mb-2'>RECIPIENT ADDRESS</label>
              <input
                type='text'
                value={state.recipientAddress}
                onChange={(e) => updateState({ recipientAddress: e.target.value, error: null })}
                placeholder='0x...'
              ...
              />
              <p className='text-zinc-600 text-xs mt-2'>Enter the recipient's stealth address or public address</p>
            </div>
            ...
            <div className='mb-6'>
              ...
              <div className='relative'>
                ...
                <div className='absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2'>
                  ...
                  <span className='text-zinc-500'>{defaultToken?.symbol ?? 'TOKEN'}</span>
                </div>
              </div>
              <p className='text-zinc-600 text-xs mt-2'>
                Available: {formatAmount(availableBalance)} {defaultToken?.symbol ?? 'tokens'}
              </p>
            </div>
            ...
            <div className='bg-zinc-800/50 border border-zinc-700 rounded-lg p-4 mb-6'>
              <div className='flex items-start gap-3'>
                ...
                <div>
                  <p className='text-white text-sm font-medium mb-1'>Private Transfer</p>
                  <p className='text-zinc-500 text-xs'>
                    This transfer is shielded with zero-knowledge proofs. The amount and recipient are hidden from the
                    public blockchain.
                  </p>
                </div>
              </div>
          ...
          <>
            <div className='text-center mb-6'>
              ...
              <h2 className='text-lg font-semibold text-white mb-2'>
                {state.isProcessing ? 'Processing Transfer...' : 'Confirm Transfer'}
              </h2>
              <p className='text-zinc-500 text-sm'>
                {state.isProcessing ? 'Please wait while the proof is generated' : 'Review the transfer details below'}
              </p>
            </div>
        ...
        {state.step === 'success' && (
          <>
            <div className='text-center mb-6'>
              ...
              <h2 className='text-lg font-semibold text-white mb-2'>Transfer Successful!</h2>
              <p className='text-zinc-500 text-sm'>Your private transfer has been completed.</p>
            </div>
